...
### 2. XcodeGen Configuration Issues - **CRITICAL FOR PROJECT RECOVERY**
**Problem**: Incorrect exclusion patterns in xcodegen.yml can exclude essential files
**Real Example from JubileeMobileBay Recovery (July 2025)**:
- `Protocols/**` exclusion prevented ALL protocol files from compiling
- Caused "undefined type" errors for WeatherAPIProtocol, MarineDataProtocol, etc.
- Created false impression of "bad architectural state"

**Symptoms**:
- Well-defined protocols show as "undefined"
- Architecture appears broken despite proper MVVM implementation
- Build errors concentrated around service layer abstractions

**Solution**:
1. Check xcodegen.yml exclusion patterns FIRST
2. Remove overly broad exclusions like `Protocols/**`
3. Be specific with exclusions (e.g., `Models/ML/CreateMLTrainer.swift`)
4. Always run `xcodegen generate` after configuration changes

**Prevention**:
- Never exclude entire functional directories
- Document all exclusions with comments explaining why
- Test build immediately after any xcodegen.yml changes

### 3. Duplicate Type Declarations
**Problem**: Legacy files and current implementations defining same types
**Real Example**: `LegacyPredictionTypes.swift` vs `PredictionModels.swift`
**Symptoms**:
- "Invalid redeclaration of 'TypeName'" errors
- "'TypeName' is ambiguous for type lookup" errors
- Compiler unable to type-check expressions

**Solution**:
1. Search for duplicate type definitions: `grep -r "struct TypeName" .`
2. Disable legacy files: rename to `.disabled` extension
3. Exclude legacy directories in xcodegen.yml: `Models/Legacy/**`
4. Regenerate project file

### 4. CreateML vs CoreML Platform Restrictions
**Problem**: CreateML training scripts included in iOS targets
**Symptoms**:
- "No such module 'CreateML'" (CreateML is macOS-only)
- "Statements are not allowed at the top level" (training scripts)

**Solution**:
- Exclude CreateML training scripts from iOS targets
- Keep only CoreML runtime files in iOS app
- Move training scripts to separate macOS playground/project

### 5. Platform Version Mismatches
